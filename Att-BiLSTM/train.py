...
def train():
    ...
    print("x = {0}".format(x.shape))#例：(8000,90)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Generate batches
            #list(zip(x_train, y_train):将每个句子与其对应的类标独热编码组成在一个列表里
            #创建一个生成器batches
            ...
            for ei,batch in enumerate(batches):
                x_batch, y_batch = zip(*batch)#x_batch：[('str1',..),(label1,..)]
                # Train
                feed_dict = {
                ...
                # Training log display每隔多少步做一次输出
                if step % FLAGS.display_every == 0:
                    time_str = datetime.datetime.now().isoformat()#当前时间
